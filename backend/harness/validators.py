...
def validate_graph(graph: dict, db: "Session | None" = None) -> list[Finding]:
    ...
    # ── nested graph structural checks (parentId, cross-scope edges, subgraph cycles) ──

    # parentId must point to a real node
    ...
    # start/end nodes cannot have a parentId
    for node_def in nodes:
    ...
    # subgraph connectivity (each subgraph needs at least one entry node)
    for pid in parent_ids:
        child_ids = {n["id"] for n in nodes if _get_parent_id(n) == pid}
        internal_edges = [
            e for e in edges
    ...
    # ── cycle detection (Kahn) — only edges within the same scope participate ──
    ...
    if node_ids:
        ...
        connected = {edge["source"] for edge in edges} | {
