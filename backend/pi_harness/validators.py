"""PIAgent Harness — Graph validation (pure functions, no LLM).
...
# Output fields each node type writes into state["node_outputs"][node_id].
# Keep in sync with backend/nodes/*_node.py.
_STATIC_NODE_FIELDS: dict[str, list[str]] = {
    "llm": ["text"],
...
def _collect_available_refs(nodes: list[dict]) -> dict[str, list[str]]:
    """Return {node_id: [valid_field_names]} for building end-node reference hints."""
    refs: dict[str, list[str]] = {}
    for node in nodes:
        node_id = node.get("id")
        node_type = node.get("type")
        if not node_id or node_type in (None, "end"):
            continue
        if node_type == "start":
            schema = get_node_config(node).get("inputs", [])
            field_names = [
                f["name"] for f in schema
                if isinstance(f, dict) and isinstance(f.get("name"), str)
            ]
            refs[node_id] = field_names
        elif node_type in _STATIC_NODE_FIELDS:
            refs[node_id] = list(_STATIC_NODE_FIELDS[node_type])
    return refs


def _format_refs_hint(refs: dict[str, list[str]]) -> str:
    if not refs:
        return "No upstream nodes produce referable outputs yet."
    parts = []
    example = None
    for node_id, fields in refs.items():
        if fields:
            parts.append(f"{{{{{node_id}.{fields[0]}}}}}" + (f" (also: {', '.join(fields[1:])})" if len(fields) > 1 else ""))
            if example is None:
                example = f"{{{{{node_id}.{fields[0]}}}}}"
        else:
            parts.append(f"{node_id} (no fields declared)")
    prefix = f"Example: {example}. " if example else ""
    return prefix + "Available references: " + "; ".join(parts) + "."
...
def validate_graph(graph: dict, db: "Session | None" = None) -> list[Finding]:
    ...
    available_refs = _collect_available_refs(nodes)
    hint = _format_refs_hint(available_refs)
                ...
                if not REF_RE.fullmatch(value):
                    findings.append(Finding(
                        ...
                        message=(
                            ...
                            f"'{out['name']}' must match {{{{nodeId.fieldName}}}} format "
                            f"(double curly braces; NOT ${{...}}, NOT single braces), got: {value!r}. "
                            f"{hint}"
                        ),
                        node_id=end_node["id"],
                    ))
                    ...
                    if match:
                        ...
                        if ref_node_id not in node_ids:
                            findings.append(Finding(
                                ...
                                message=(
                                    f"End node '{end_node['id']}' reference output "
                                    f"points to unknown node: {ref_node_id}. {hint}"
                                ),
                                node_id=end_node["id"],
                            ))
                        elif ref_field and ref_node_id in available_refs:
                            valid_fields = available_refs[ref_node_id]
                            if valid_fields and ref_field not in valid_fields:
                                findings.append(Finding(
                                    severity="error",
                                    code="bad_template_ref",
                                    message=(
                                        f"End node '{end_node['id']}' reference output "
                                        f"'{out['name']}' uses unknown field '{ref_field}' on node "
                                        f"'{ref_node_id}'. Valid fields on '{ref_node_id}': "
                                        f"{sorted(valid_fields)}. {hint}"
                                    ),
                                    node_id=end_node["id"],
                                ))
